...
export default function Section5() {
  ...
  return (
    <footer ref={containerRef} className="h-screen w-full bg-[#0C0F12] text-[#F9F9F7] flex flex-col items-center justify-center relative overflow-hidden border-t border-[#F9F9F7]/10 cursor-none">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center opacity-10 pointer-events-none w-[120vw] -left-[10vw]">
        ...
        <h1 ref={text2Ref} className="text-[25vw] font-serif leading-none tracking-tighter -mt-[10vw] text-[#C5A880] w-full text-center">TALK</h1>
      </div>
      ...
      <div className="z-10 text-center flex flex-col items-center gap-12 mt-16 pointer-events-none">
        <h2 className="text-3xl md:text-5xl font-serif max-w-xl leading-tight">
          Ready to elevate your space into a <span className="italic text-[#C5A880]">masterpiece?</span>
        </h2>
        ...
        >
          <div className="absolute inset-0 bg-[#C5A880] opacity-0 group-hover:opacity-10 scale-50 group-hover:scale-100 rounded-full transition-all duration-700 blur-xl"></div>
          <span ref={btnTextRef} className="font-mono text-xs tracking-widest uppercase group-hover:text-[#0C0F12] transition-colors duration-500 z-10 font-bold">
            Start Project
          </span>
        </Link>
      </div>
      ...
      <div className="absolute bottom-0 w-full px-8 md:px-16 py-8 flex flex-col md:flex-row justify-between items-center gap-4 text-[0.55rem] tracking-widest font-mono uppercase opacity-50 z-20">
        <div className="flex gap-8">
          <span>Ãƒâ€šÃ‚Â© 2026 DESIGN SOLUTIONS MIAMI</span>
        </div>
        ...
        <div>
          <span>MADE BY <span className="text-[#C5A880]">.ME PRODUCTIONS</span></span>
        </div>
      </div>
    </footer>
  );

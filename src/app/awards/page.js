...
export default function Awards() {
  return (
    <SmoothScroll>
      ...
      <main className="min-h-screen w-full bg-[#050505] text-[#F9F9F7] flex flex-col pt-32 pb-24 px-8 md:px-16 lg:px-24 selection:bg-[#D4AF37] selection:text-[#050505]">
          ...
          <div className="w-full flex flex-col border-t border-white/20">
            {awardsData.map((award, index) => (
              <motion.div 
                ...
                className="group flex flex-col md:flex-row md:items-center justify-between py-10 border-b border-white/10 hover:border-[#D4AF37]/50 transition-colors duration-500 relative overflow-hidden"
              >
                {/* Hover Background Sweep */}
                <div className="absolute inset-0 bg-gradient-to-r from-[#D4AF37]/0 via-[#D4AF37]/5 to-[#D4AF37]/0 opacity-0 scale-y-0 group-hover:opacity-100 group-hover:scale-y-100 origin-bottom transition-all duration-700 pointer-events-none"></div>

                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-16 relative z-10 w-full md:w-3/4">
                  <span className="font-mono text-sm tracking-widest text-white/40 group-hover:text-[#D4AF37] transition-colors duration-500 w-16">
                    {award.year}
                  </span>
                  <div className="flex flex-col">
                    <h3 className="text-3xl md:text-5xl font-serif tracking-tight group-hover:italic group-hover:text-[#D4AF37] transition-all duration-500">
                      {award.title}
                    </h3>
                ...
                <div className="mt-6 md:mt-0 relative z-10 text-left md:text-right">
                  <span className={`px-4 py-2 rounded-full border text-[0.6rem] font-bold tracking-[0.2em] uppercase ${award.result === 'Winner' ? 'border-[#D4AF37] text-[#D4AF37]' : 'border-white/20 text-white/50 group-hover:border-white/50'}`}>
                    {award.result}
                  </span>
                </div>
